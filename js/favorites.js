const FavoritesManager = {
    ...
    handleBookmarkClick: function(projectId, button, card) {
        ...
        const projectData = {
            ...
            tech: Array.from(card.querySelectorAll('.tech-stack-terminal span')).map(span => span.textContent),
    ...
    updateBookmarkButton: function(projectId, isFavorite) {
        const bookmarkBtn = document.querySelector(`.project-bookmark-btn[data-project-id="${projectId}"]`);
        if (bookmarkBtn) {
            if (isFavorite) {
                bookmarkBtn.classList.add('active');
            } else {
    ...
    filterFavorites: function() {
        ...
        this.favorites.forEach(projectId => {
            const card = document.querySelector(`.cyber-card.project-card[data-project-id="${projectId}"]`);
            if (card) {
